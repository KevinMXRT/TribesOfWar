...
bindEvents: function() {
  $(document).on('click', '.cardCollect', App.createWarrior(
    $("#warName").val(),
    $("#level").val(),
    $("#attack").val(),
    $("#defend").val(),
...
web3.eth.getAccounts(function(error, accounts) {
  ...
  App.contracts.WarriorFactory.deployed().then(function(instance) {
    ...
    return WarriorFactoryInstance.createWarrior(warName, level, attack, defend, strategy, winCount, lossCount, tokenURI, to);
  }).catch(function(err) {
